...
<body>
    ...
    <main class="container">
        <div class="header-flex"
            ...
            <a href="dashboard.php"
                style="background: #2c3e50; color: #fff; padding: 6px 14px; border-radius: 4px; text-decoration: none; font-size: 13px;">&larr;
                Kembali ke Dashboard</a>
        </div>

        <div class="filter-box"
            style="background: #fff; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #ddd; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            <form method="GET" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;">
                <div>
                    ...
                    <select name="f_akun" class="inline-select">
                        ...
                        <option value="<?=$a['id']?>" <?=($filter_akun_id) == $a['id'] ? 'selected' : ''?>>
                            <?=$a['nama_akun']?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="f_kategori" class="inline-select">
                        ...
                        <option value="<?=$k['id']?>" <?=($_GET['f_kategori'] ?? '') == $k['id'] ? 'selected' : ''?>>
                ...
                <div>
                    ...
                    <select name="f_tipe" class="inline-select">
                        ...
                        <option value="KELUAR" <?=($_GET['f_tipe'] ?? '') == 'KELUAR' ? 'selected' : ''?>>Keluar
                        </option>
                    </select>
                </div>
                <div>
                ...
                <button type="submit"
                    style="padding: 8px 15px; cursor: pointer; background: #2c3e50; color: white; border: none; border-radius: 4px;">Filter</button>
                <a href="riwayat_lengkap.php"
                    style="padding: 8px 15px; background: #eee; text-decoration: none; color: #333; border-radius: 4px; display: inline-block;">Reset</a>
            </form>
            ...
            <div style="display: flex; gap: 10px; margin-top: 12px;">
                ...
                <a href="transaksi/cetak_excel.php?<?php echo http_build_query($_GET); ?>" onclick="if(confirm('Apakah Anda ingin mendownload laporan Excel ini?')) { 
       setTimeout(function() { window.location.href = 'riwayat_lengkap.php'; }, 1000); 
       return true; 
   } else { 
       return false; 
   }" style="padding: 8px 15px; background: #2c3e50; color: white; text-decoration: none; border-radius: 4px; display: inline-block;">
            ...
            <div class="month-group-container">
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            <?php 
    // Hitung saldo awal masing-masing akun SEBELUM bulan ini dimulai
    $tanggal_awal_bulan = date('Y-m-01', strtotime($transaksi_grup[0]['tanggal']));
    $saldo_akun_sebelum_bulan = [];
    $total_saldo_sebelum_bulan_ini = 0;

    foreach ($tampil_akun as $akun) {
        $acc_id = $akun['id'];
        $q_sebelum = mysqli_query($conn, "
            SELECT 
                COALESCE(SUM(pemasukan), 0) AS total_masuk,
                COALESCE(SUM(pengeluaran), 0) AS total_keluar
            FROM transaksi
            WHERE user_id = $user_id AND akun_id = $acc_id 
              AND tanggal < '$tanggal_awal_bulan'
        ");
        $d_sebelum = mysqli_fetch_assoc($q_sebelum);
        $mutasi_lalu = (float)$d_sebelum['total_masuk'] - (float)$d_sebelum['total_keluar'];
        
        // Saldo akun ini sebelum bulan berjalan
        $saldo_satu_akun = (float)$saldo_awal_map[$acc_id] + $mutasi_lalu;
        $saldo_akun_sebelum_bulan[$acc_id] = $saldo_satu_akun;
        
        // Akumulasi total saldo gabungan seluruh akun
        $total_saldo_sebelum_bulan_ini += $saldo_satu_akun;
    }
    ?>
                            ...
                            <tr style="background-color: #f8f9fa; font-weight: bold; font-style: italic;">
                                ...
                                <td colspan="5" style="text-align: left;">
                                    Saldo Terakhir Bulan <?php echo $bulan; ?>
                                </td>

                                <?php foreach ($tampil_akun as $akun): ?>
                                <td>Rp
                                    <?php echo number_format($saldo_akun_sebelum_bulan[$akun['id']] ?? 0, 0, ',', '.'); ?>
                                </td>
                                <?php endforeach; ?>
                                ...
                                <td style="text-align: right;">
                                    <strong>Rp
                                        <?php echo number_format($total_saldo_sebelum_bulan_ini, 0, ',', '.'); ?></strong>
                                </td>
                                <td colspan="2"></td>
                            </tr>
        ...
        ?>
                            <tr>
                                <td><input type="checkbox" class="row-checkbox"
                                        data-masuk="<?php echo (float)$tr['pemasukan']; ?>"
                                        data-keluar="<?php echo (float)$tr['pengeluaran']; ?>"></td>
                                <td class="edit-cell" data-field="tanggal" data-id="<?php echo (int) $tr['id']; ?>"
                                    data-value="<?php echo htmlspecialchars($tr['tanggal'], ENT_QUOTES); ?>"
                                    data-type="date">
                                    <?php echo htmlspecialchars($tr['tanggal']); ?>
                                </td>
                                <td class="edit-cell" data-field="deskripsi" data-id="<?php echo (int) $tr['id']; ?>"
                                    data-value="<?php echo htmlspecialchars($tr['deskripsi'], ENT_QUOTES); ?>"
                                    data-type="text">
                                    <?php echo htmlspecialchars($tr['deskripsi']); ?>
                                </td>
                                <td class="edit-cell" data-field="kategori_id" data-id="<?php echo (int) $tr['id']; ?>"
                ...
                ?>
                                    ...
                                    <div class="overlay-bukti"
                                        ...
                                        <div
                                            style="position: relative; max-width: 95%; max-height: 95%; text-align: center;">
                                            <span onclick="tutupModal(this)"
                                                style="position: absolute; top: -45px; right: 0; font-size: 35px; color: white; cursor: pointer; font-weight: bold; background: rgba(0,0,0,0.6); width: 42px; height: 42px; text-align: center; line-height: 40px; border-radius: 50%;">&times;</span>
                                            <img src="<?php echo htmlspecialchars($file_path); ?>" alt="Bukti"
                                                style="max-width: 100%; max-height: 85vh; border-radius: 8px;">
                                        </div>
                                    </div>
                                    <?php elseif ($is_zip): ?>
                                    ...
                                    <div class="overlay-bukti"
                                        style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.85); z-index: 9999; justify-content: center; align-items: center; padding: 20px;">
                                        <div
                                            style="position: relative; width: 600px; max-width: 95%; background: white; padding: 25px; border-radius: 8px; max-height: 85vh; overflow-y: auto;">
                                            <span onclick="tutupModal(this)"
                                                style="position: absolute; top: 12px; right: 18px; font-size: 28px; cursor: pointer;">&times;</span>
                                ...
                                <td>
                                    <a href="hapus_transaksi.php?id=<?php echo $tr['id']; ?>"
                                        onclick="return confirm('Yakin hapus? Saldo akan terkoreksi.')"
    ...
    <div id="calculator-box"
        style="display: none; position: fixed; bottom: 20px; right: 20px; background: #2c3e50; color: white; padding: 15px 25px; border-radius: 50px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); z-index: 1000; font-weight: bold;">
    ...
    function tutupModal(el) {
        var ov = el.closest('.overlay-bukti');
